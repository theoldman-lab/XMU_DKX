...
\begin{frame}{数据集与预处理}
    \begin{columns}[T]
        ...
        \begin{block}{数据集}
            \footnotesize
            BCI Competition IV 2a \cite{tangermann2012review}
            \begin{itemize}
...
\begin{frame}{特征提取与分类}
    \begin{columns}[T]
        ...
        \begin{block}{CSP 特征提取}
            \footnotesize
            \vspace{0.1cm}
            \textbf{共空间模式 (CSP)}\cite{blankertz2008optimizing,mcfarland1997spatial}：使不同类别信号方差差异最大化
        ...
        \begin{block}{分类器设计}
            \footnotesize
            \textbf{分类器}\cite{lotte2007review}：线性核 SVM ($C=1.0$)
...
\begin{frame}{对比方法与评估协议}
    \begin{columns}[T]
        ...
        \begin{block}{评估协议}
            \footnotesize
            \cite{tangermann2012review,cohen1960coefficient}
